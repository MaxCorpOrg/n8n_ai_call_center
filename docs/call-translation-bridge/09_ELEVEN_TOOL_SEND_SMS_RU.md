...
- для `short_info` используется SMS с контактами менеджеров для связи и консультации;
- для `product_intro` используется SMS с коротким объяснением, что такое LipoLong, ключевыми преимуществами, ориентиром по цене, условиями входа и контактами менеджеров для консультации;
- номер нормализуется в формат `+7...`, если это российский номер;
- текст ограничивается длиной `MANGO_SMS_MAX_LENGTH`.

Текущий `short_info` SMS-блок:
...
Текущий `product_intro` SMS-блок:
...
- `short_info`
- `product_intro`
  - использовать, если клиент просит рассказать, что это за продукт, какие преимущества, какая цена или как вообще зайти в направление
- `callback_confirmation`
- `offer`
  - сохраняется как backward-compatible alias и внутри workflow маппится в `product_intro`
...
- Если клиент просит только контакты, менеджера или способ связи, используй message_intent = short_info.
- Если клиент просит объяснить, что такое LipoLong, прислать преимущества, цену или информацию по препарату, используй message_intent = product_intro.
- Если клиент не использует инъекционные методики, но направление коррекции фигуры для него релевантно, предлагай product_intro: там есть краткое описание продукта, ориентир по цене и контакты менеджеров для консультации.
