...
- `Здравствуйте.` не должно склеиваться с продажным opener в одну реплику
- после `Здравствуйте.` агент ждёт ещё один короткий живой ответ (`да`, `слушаю`, `добрый день`, `алло`) и только потом переходит к бизнес-опенеру
- текущий бизнес-opener после этого:
...
- следующий короткий вопрос после opener:
