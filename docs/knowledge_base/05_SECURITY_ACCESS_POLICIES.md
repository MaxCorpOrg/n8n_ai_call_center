...
## 2) Актуальные меры
- SSH:
  - актуальный операционный вход на прод-сервер `147.45.213.87` выполняется по выделенному SSH-ключу;
  - рекомендуемый alias на машине `max`: `ssh ai-core-prod-147`;
  - ручная зависимость от root-пароля для повседневной работы снята.
