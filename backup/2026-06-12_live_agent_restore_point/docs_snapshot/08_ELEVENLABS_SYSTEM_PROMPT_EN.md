...
## Live note 2026-06-12

- Current confirmed live version:
  - `agtvrsn_0901ktxsrethemqr4prhkw701wr2`
- Current fixed opener in live:
...
- Current live second-turn rule after opener:
  - maximum `2` short sentences;
  - maximum `1` simple next-step question;
  - compact follow-up templates are explicitly embedded in the live prompt for:
    - neutral / lightly positive answer;
    - `what is the call about?`;
    - busy response;
    - clearly live intermediary handoff.
- Current live refusal rule right after the opener:
  - a flat immediate negative like `нет`, `не надо`, `не нужно`, `неинтересно` is no longer treated as final by default;
  - the agent should first clarify whether this is `not_target` or just low current interest;
  - if the contact is relevant, the agent should make one compact rescue move such as SMS or a manager callback;
  - only after the clarification plus one rescue move may the call end as `refusal_soft`.
- The long historical prompt body below is still useful as context, but the live opener wording and second-turn behavior should be taken from this note and from the latest checkpoints first.
