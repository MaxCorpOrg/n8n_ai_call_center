...
- после разбора этого звонка live-agent ещё раз пропатчен:
  - текущая live version:
    - `agtvrsn_5001ktxqwz6jer28593yds2asped`
  - новое правило:
    - flat immediate negative (`нет`, `не надо`, `не нужно`) сразу после opener не rescue'им;
    - делаем `call_log(refusal_soft)` и silent end без spoken farewell.
- тестовый outbound-контур снова снят с публикации;
- новый звонок после refusal-fix ещё не запускался.
- артефакты:
