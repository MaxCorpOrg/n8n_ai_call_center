...
- после этого поверх live внесён новый refusal-fix:
  ...
  - flat immediate negative сразу после opener теперь нужно трактовать как финальный отказ на этот звонок;
  - в таком кейсе агент не должен rescue'ить ответ и не должен говорить closing phrase;
  - должен быть только `call_log(refusal_soft)` и silent end.
- новый test-call после этого refusal-fix ещё не запускался.
