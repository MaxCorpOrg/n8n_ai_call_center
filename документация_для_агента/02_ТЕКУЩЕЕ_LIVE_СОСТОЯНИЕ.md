...
- `2026-05-25 после 16:00 MSK`: git-линия проекта приведена в порядок для следующего цикла:
  - `origin/main` синхронизирован с рабочей веткой и сейчас на `c6a25ec`;
  - серверный clean-clone `/home/aicore/n8n-ai-clean` тоже обновлён до `c6a25ec`;
  - для отдельного техцикла создан отдельный worktree:
    - `/home/max/worktrees/n8n-postgres-migration`
    - ветка: `codex/n8n-postgres-migration`
  - локальный старый каталог `/home/max/n8n_ai_call_center` остаётся operational dirty и не должен использоваться как чистая точка старта для нового большого цикла.
- Окно обзвона:
  - `10:00–14:00 MSK`
- Текущие ограничения:
