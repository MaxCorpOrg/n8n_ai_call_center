...
- live-agent уже ужат по ожиданию:
  - фактический live `Main` всё ещё нужно дожать до `turn_timeout = 5.0` через Eleven UI/API из разрешённой сети;
...
- отдельное ужесточение по intermediary-линии:
  - `я передам ответственному специалисту`, `оставьте контакт`, `мы передадим информацию`, `я только передам` не считать полезным human-contact;
  - не оставлять pitch и не логировать такие случаи как успешную передачу сообщения;
  - завершать как blocked/non-human intermediary outcome.
