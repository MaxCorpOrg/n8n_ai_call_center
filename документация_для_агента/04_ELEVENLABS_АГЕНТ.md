...
- live-agent уже ужат по ожиданию:
  - фактический live `Main` уже работает с `turn_timeout = 5.0`;
...
- правило по message-service:
  ...
  - прямой API-доступ с `147.45.213.87` всё ещё ограничен `302/403`, но рабочий patch-path подтверждён через relay-хост `151.241.228.232`;
