...
if [[ $# -lt 2 || $# -gt 3 ]]; then
  cat <<'EOF' >&2
Usage:
  prepare_eleven_interruptible_softfill_variant.sh SOURCE_AGENT_JSON OUTPUT_JSON [VERSION_DESCRIPTION]

Builds a lab-only payload for:
  - interruptions enabled
  - less aggressive turn-taking
  - softer filler masking without time-promise phrasing

Optional environment overrides:
  TARGET_TURN_TIMEOUT      default: 2.3
  TARGET_TURN_EAGERNESS    default: normal
  TARGET_SOFT_TIMEOUT      default: 2.4
...
TARGET_SOFT_TIMEOUT="${TARGET_SOFT_TIMEOUT:-2.4}"
