      ...
      (.conversation_config.agent.prompt.tools
       | map(
           if .name == "call_log" then
             .description = "Internal call outcome logging tool. For terminal outcomes like refusal_soft, not_target, no_answer, busy, machine-stop, or completed SMS wrap-up, call this silently before any final spoken close. Do not reopen the dialogue after call_log. Do not manually fabricate conversation_id or eleven_conv_id; let system-bound fields fill them."
           elif .name == "end_call" then
             .description = "Internal end-call tool. Use it immediately after silent call_log for terminal outcomes. The one and only allowed final spoken close must live only inside end_call.system__message_to_speak. Never say the same close first as a normal assistant turn and then again in end_call. Never produce a normal assistant reply after call_log."
           else
             .
           end
         ))
